 ...
 * File name: MainActivity.java[Inclass05.app]
 * Names of the students: Aakanksha Chauhan, Sindhura Chaganti
 */
public class MainActivity extends AppCompatActivity implements LoginFragment.AccountListener, RegisterFragment.RegisterListener, AppCategoryFragment.AppListener, AppListFragment.AppListListener {
    ...
    @Override
    public void afterRegistration(String accountId) {
        //Will open app category page after successful registration
        token = accountId;
        getSupportFragmentManager().beginTransaction()
                .replace(R.id.containerView, AppCategoryFragment.newInstance(token))
                .commit();
    }
    ...
    public void setAppListFragment(String appCategory) {
        ...
        getSupportFragmentManager().beginTransaction()
                .replace(R.id.containerView, AppListFragment.newInstance(this.token, appCategory))
                .addToBackStack(null)
                .commit();
    }
